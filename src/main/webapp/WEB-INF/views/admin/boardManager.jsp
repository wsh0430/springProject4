	...
	<div id="manager-bar">
		<form name="searchForm" method="get">
			...
			<div id="m_control-panel">
				<input type="button" id="m_hide-all-button" value="숨김" onclick="updateToggleAllMItem('board','hide')" style="display: none;">
				<input type="button" id="m_show-all-button" value="보이기" onclick="updateToggleAllMItem('board', 'show')" style="display: none;">
				<input type="button" id="m_delete-all-button" value="삭제" onclick="deleteCheckedAllMItem('board')" style="display: none;">
			</div>
	</div>
	<div id="manager-box">
		<table id="m-list">
			...
			<c:forEach var="bVos" items="${boardVos}">
				<tr style="background-color: #e2e2e2;">
					<td><input type="checkbox" class="ml_checkbox" id="${bVos.idx}"></td>
					<td>${bVos.categoryName}</td>
    				<td>
  						<c:choose>
						  <c:when test="${fn:length(bVos.title) > 25}">
						    <a href="${ctp}/community/cmtyContent?boardIdx=${bVos.idx}">
    				...
    				<td>
    					<div class="mi_btn">
	    					<div id="mi_toggle-button${bVos.idx}">					
		    					<c:if test="${bVos.hideCheck == 0}"><input type="button" id="mi_hide-button${bVos.idx}" value="숨김" onclick="updateToggleMItem('board', 'hide', ${bVos.idx})"></c:if>
		    					<c:if test="${bVos.hideCheck != 0}"><input type="button" id="mi_show-button${bVos.idx}" value="보이기" onclick="updateToggleMItem('board', 'show', ${bVos.idx})"></c:if>
	    					</div>
	    					<input type="button" id="mi_delete-button" value="삭제" onclick="deleteMItem('board', '${bVos.title}', ${bVos.idx})">
    					</div>
